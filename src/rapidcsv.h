...
namespace rapidcsv {
	...
	struct SeparatorParams {
		...
		explicit SeparatorParams(const char pSeparator = ',', const bool pHasCR = sPlatformHasCR)
				: mSeparator(pSeparator), mHasCR(pHasCR) {
		}
	...
	public:
		...
		explicit Document(const std::string &pPath = std::string(),
						  const bool enforce_column_type = false,
						  const LabelParams &pLabelParams = LabelParams(),
						  const SeparatorParams &pSeparatorParams = SeparatorParams(),
						  const ConverterParams &pConverterParams = ConverterParams())
				: mPath(pPath), mLabelParams(pLabelParams), mEnforceColumnType(enforce_column_type),
		...
		explicit Document(const Document &pDocument)
				: mPath(pDocument.mPath), mLabelParams(pDocument.mLabelParams),
				  ...
				  mEnforceColumnType(pDocument.mEnforceColumnType),
		...
		std::vector<T> GetColumn(const size_t pColumnIdx) const {
			...
			{
				if (!mEnforceColumnType &&
					std::distance(mData.begin(), itRow) > mLabelParams.mColumnNameIdx)
				{
					T val;
		...
		std::vector<T> GetRow(const size_t pRowIdx) const {
			...
			{
				if (!mEnforceColumnType &&
					std::distance(mData.at(rowIdx).begin(), itCol) > mLabelParams.mRowNameIdx)
				{
					T val;
